...
async function getByOrderId(){
    ...
    payDto.value.orderId = orderList.value[0].orderId
    payDto.value.scIds = route.query.scIds
    console.log(payDto.value)
}
...
async function pay(){
    let {data} = await request.post("/order/pay",payDto.value)
...
async function remove(){
    await request.get("/order/remove",{params:{'orderId':route.query.orderId}})
    ElMessage
}
...
<template>
    <div class="base_div">
        ...
        <div style="width:80%;display:flex;justify-content:flex-end">
            <el-button style="width:210px;height:70px;color:white" @click="remove" color="rgb(155,155,155)">取消订单</el-button>
